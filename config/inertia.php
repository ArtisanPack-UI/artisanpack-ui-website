...
/*
 * Every root that can own an Inertia page: the core `resources/js/pages`
 * plus one per extracted module (plans/14-modular-laravel-setup.md §3.4).
 * The frontend flattens the same two roots into a single page-key map in
 * `resources/js/lib/resolve-page.ts`; this list is the server-side mirror
 * of it, and it is what makes `assertInertia()->component('admin/...')`
 * resolve for a page that has moved into a module. Without the module
 * entries every module page assertion fails with "page component file
 * does not exist" even though the page renders correctly in the browser.
 *
 * Under Inertia v3 this feeds `pages.paths` (the old top-level `page_paths`
 * and `testing.page_paths` keys were removed); the `inertia.view-finder`
 * binding and `AssertableInertia::component()` both resolve against it.
 *
 * Globbed rather than listed so a new module needs no edit here. Config is
 * cached on deployed sites, so the glob runs once at cache time.
 */
$pagePaths = array_merge(
...
return [
    ...
    */
    ...
    | components and which file extensions to consider. `paths` is the shared
    | core + per-module glob defined above.
    |
    */
