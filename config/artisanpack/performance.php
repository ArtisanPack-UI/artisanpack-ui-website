...
return [
    ...
    */
    ...
    'routes' => [
        ...
        //
        // Keystone override: the vendor default omits VerifyCsrfToken.
        // For an authenticated admin viewing the CacheManagement page,
        // that means a POST to `/api/performance/admin/cache/actions`
        // (flush, warm, invalidate-key, invalidate-tag) rides on the
        // session cookie alone — a CSRF surface an attacker page can
        // trigger via a form submit despite SameSite=lax when there's
        // a same-site XSS foothold or subdomain takeover. Adding
        // `PreventRequestForgery` closes the gap; the React
        // CacheManagement page reads the `csrf-token` meta emitted by
        // `resources/views/app.blade.php` and sends it as
        // `X-CSRF-TOKEN`.
        'admin_middleware' => [
