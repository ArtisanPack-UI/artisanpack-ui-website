...
use Modules\Users\Database\Factories\UserEditorPreferenceFactory;
...
class UserEditorPreference extends Model
{
    ...
    public static function payloadForPreference(string $postType, ?self $preference): array
    {
        return [
            ...
            // A missing row (and any out-of-range stored value) reads back as
            // the shipped default — the switcher never has to special-case a
            // first-time editor. See {@see EditorViewModes::normalize()}.
            'view_mode'        => EditorViewModes::normalize($preference?->view_mode),
        ];
    }
    ...
    /**
     * Resolve the model's factory explicitly — see
     * {@see User::newFactory()} for why the default guesser can't.
     */
    protected static function newFactory(): UserEditorPreferenceFactory
    {
        return UserEditorPreferenceFactory::new();
    }
