...
/**
 * Registers the Analytics module's routes.
 *
 * The module owns one admin screen and no public or API surface — the
 * `artisanpack-ui/analytics` package registers its own tracking and
 * dashboard endpoints — so only `routes/admin.php` is mapped, and the
 * generated `routes/{web,api}.php` were deleted along with their
 * mapping methods.
 *
 * The group below reapplies verbatim the outer group the central
 * `routes/admin.php` used to wrap this route in: the `web` middleware
 ...
 * stops resolving. The `feature:analytics` gate stays inside
 * `routes/admin.php`, where it is legible next to the route it guards.
 */
