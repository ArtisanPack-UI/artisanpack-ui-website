...
export function WidgetSettingsModal({
    ...
    // The field names the schema actually renders a row for — used to route
    // unmatched server validation keys to the form-level slot in
    // {@link normalizeErrors} rather than dropping them.
    ...
    //
    // The previous signature is held in state rather than a ref on purpose.
    // React may discard a render and replay it; a ref written during render
    // survives that discard, so the signature would read as already-seeded
    // while the `setValues` reset it was supposed to pair with never
    // committed — leaving the form on the previous widget's options. State is
    // thrown away with the abandoned render, which is React's documented
        ...
        router.patch(
            ...
            { options: values } as never,
            {
                ...
                onError: (responseErrors) => {
                    setErrors(normalizeErrors(responseErrors, fieldNames));
                },
            ...
            >
                ...
                <form onSubmit={handleSubmit} className="flex flex-col">
                    ...
                    <footer className="flex items-center justify-end gap-2 border-t border-base-300/60 px-5 py-3">
                        ...
                        <button
                            type="submit"
                            className="rounded-md bg-primary px-3 py-1.5 text-xs font-semibold text-primary-content hover:bg-primary-hover disabled:opacity-50"
                            disabled={saving}
                        >
                            {saving ? 'Saving…' : 'Save'}
                        </button>
...
/**
 * Map server validation keys onto the field names `FieldRow` looks up.
 *
 * `fieldNames` is the set the schema actually renders. Anything that doesn't
 * land in it falls through to `__form__` rather than being written under a key
 * no row reads — otherwise the message is silently dropped and the modal shows
 * a failed save with nothing explaining it. That is reachable today: Laravel
 * nests keys on the dot, so a field literally named `range.min` arrives as
 * `options.range.min` and strips to `range.min`, which matches; but a field
 * whose name contains a dot *and* validates as a nested array does not, and
 * neither does any key the server invents that the schema has no row for.
 */
