...
export function AddWidgetDrawer({
    ...
    // Matches WidgetSettingsModal / WidgetLayoutPopover: `aria-modal="true"`
    // promises focus stays inside, and without a trap Tab walks straight out
    // into the dashboard behind the overlay.
    ...
    useEffect(() => {
        ...
        // browser's scroll-into-view behavior on the input. The delay also
        // keeps this winning over `useFocusTrap`, which focuses the panel's
        // first focusable synchronously on activation — the search box is the
        // better landing spot here. Don't collapse this to a direct call.
                                    ...
                                    <ul className="flex flex-col gap-2">
                                        {entries.map(({ type, widget }) => (
                                            ...
                                            >
                                                ...
                                                <button
                                                    type="button"
                                                    className="self-start rounded-md bg-primary px-3 py-1.5 text-xs font-semibold text-primary-content hover:bg-primary-hover disabled:opacity-50"
