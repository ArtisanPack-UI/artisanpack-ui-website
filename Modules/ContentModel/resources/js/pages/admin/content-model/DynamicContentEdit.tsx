...
import { useEditorViewMode } from '@/components/admin/editor/useEditorViewMode';
import { DEFAULT_EDITOR_VIEW_MODE } from '@/lib/admin/editorChrome';
...
interface PageProps extends KeystoneSharedProps {
    ...
    /** Per-user editor chrome view mode, keyed by content-type slug (#239). */
...
export default function DynamicContentEdit() {
    ...
    // Gate Save until the visual editor has finished mounting (issue #237):
    // a click that lands mid-mount was silently dropped. Tracks WHICH record's
    // editor reported ready rather than a bare boolean, so a same-component
    // navigation to another record re-gates Save until the new boot settles
    // (see posts/Edit.tsx). The `canSubmit` derivation below folds in the
    // no-editor and plugin-renderer cases (nothing of ours to wait for).
    ...
    // The visual editor node, routed through the same plugin filter as the
    // mount below so a plugin can substitute a different editor — or return
    // null to suppress it. The view-mode switcher keys off whether this
    // resolves to an actual node, not merely off `supports('editor')`: an
    // older table missing the `content` column, or a plugin renderer that
    // returns null, means no editor and therefore no switcher (issue #239).
    const builtInEditorNode: ReactNode = showContent ? (
        <VisualEditor
            ...
            onReady={() => setReadyRecordId(record.id as number)}
        />
    ) : null;
    const editorNode: ReactNode = showContent
        ...
        : null;
    // Every falsy node counts as "no editor", not just null/undefined: a
    // plugin renderer can veto with `false` (the established suppression value
    // for these filters, mirroring the `beforeSubmit` vetoes), and `''`/`0`
    // render nothing either — none should surface the switcher or hide chrome.
    const editorEnabled = Boolean(editorNode);
    // Only our built-in editor reports readiness through `onReady`, so gate
    // Save on it only when that exact node is what the filter returned. A
    // plugin that replaces the editor owns its own lifecycle, and one that
    // wraps ours (`<X>{builtInEditorNode}</X>`) changes the reference too —
    // both fall back to ungated Save, i.e. pre-#237 behaviour, never worse.
    // No editor at all means nothing to wait for. Gating on an `onReady`
    // that will never arrive would strand Save disabled, so we don't.
    const builtInEditorMounted = editorEnabled && editorNode === builtInEditorNode;
    const canSubmit = !builtInEditorMounted || readyRecordId === (record.id as number);

    // This shell is single-column (no 320px settings sidebar), so
    // distraction-free hides the admin chrome only and keeps this page's own
    // header — with the switcher and Save button — reachable. There is no
    // slide-over and no floating control here (`hasSettingsPanel: false`).
    const viewModeControl = useEditorViewMode({
        postType: contentType.slug,
        initialMode: editorPreferences?.view_mode ?? DEFAULT_EDITOR_VIEW_MODE,
        ...
        <>
            ...
            <div className="flex flex-col gap-7">
                        ...
                        <>
                            {editorEnabled && (
                                <EditorViewModeSwitcher
                                    mode={viewModeControl.mode}
                                    onChange={viewModeControl.setMode}
                                    activeButtonRef={viewModeControl.switcherButtonRef}
                                />
                            )}
                            {/*
                             * Native `disabled` while the editor mounts
                             * (issue #237) so a click — or an Enter that
                             * would implicitly submit this associated form —
                             * can't be silently dropped in the mount window.
                             * `aria-busy` names the transient wait; the label
                             * swaps to a self-describing "Preparing…", the
                             * same idiom this button already uses for saving.
                             */}
                            <button
                                type="submit"
                                form="dynamic-content-form"
                                disabled={form.processing || !canSubmit}
                                aria-busy={form.processing || !canSubmit ? true : undefined}
                            ...
                            >
                                {form.processing ? 'Saving…' : canSubmit ? 'Save changes' : 'Preparing…'}
                            </button>
                        </>
                    }
                />
                ...
                <form id="dynamic-content-form" onSubmit={handleSubmit} className="flex flex-col gap-5">
                                        ...
                                        <div key={tax.slug}>
                                            ...
                                            <div className="mt-2 flex items-center gap-2">
                                                ...
                                                <button
                                                    ...
                                                    className="inline-flex items-center gap-1.5 rounded-lg border border-base-300 px-3 py-1.5 text-xs font-semibold text-base-content/75 hover:bg-base-200"
                                                >
                                                    Add
                                                </button>
                                            </div>
                    ...
                    {/*
                     * The visual editor mount (or a plugin substitute) is
                     * computed once above as `editorNode` — both this render
                     * and the view-mode switcher's gating read the same value
                     * through `keystone.admin.dynamicContent.registerRenderer`
                     * so they can never disagree about whether an editor is
                     * present. Args: `(ReactNode, { contentType, record, form })`.
                     */}
                    {editorNode}
                </form>
            </div>
