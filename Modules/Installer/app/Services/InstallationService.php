...
use Modules\Installer\Support\InstallationReport;
...
class InstallationService
{
    ...
    public function install(InstallationOptions $options): InstallationReport
    {
        ...
        $this->ensureRuntimeDirectories($report);
    ...
    /**
     * Create the framework runtime directories before anything touches the
     * cache, session, or view compiler. A release zip strips the empty dirs
     * (their `.gitkeep`s match the CI exclude globs — see #115), so on a fresh
     * unzip these paths can be missing and the first cache write would throw
     * "Please provide a valid cache path". Belt-and-braces alongside the CI fix.
     */
    private function ensureRuntimeDirectories(InstallationReport $report): void
    {
        $directories = [
        ...
        try {
            foreach ($directories as $directory) {
                ...
                $relative = Str::after($directory, storage_path().DIRECTORY_SEPARATOR);

                // makeDirectory() can return false (mkdir failing without
                // throwing) as well as throw — treat a false result as a hard
                // failure so we never report `ok` for a directory that was not
                // actually created.
                if (! File::makeDirectory($directory, 0755, recursive: true)) {
                    $report->step('runtime_dirs', 'failed', "Could not create {$relative}");
            ...
            // A filesystem failure (e.g. an unwritable storage path) must
            // surface as a structured `failed` step, not escape and crash the
            // command — the workflow is built around the report and
            // InstallCommand only catches option-validation errors.
            $report->step('runtime_dirs', 'failed', $e::class.': '.$e->getMessage());

            return;
        }

        if ([] === $created) {
            $report->step('runtime_dirs', 'skipped', 'all runtime directories present');

            return;
        }

        $report->step('runtime_dirs', 'ok', 'created '.implode(', ', $created));
    }
    ...
    private function generateSitemap(InstallationReport $report): void
    {
        try {
            ...
            app(SitemapEntryReconciler::class)->apply();
