...
use Modules\Plugins\Exceptions\PluginUpdateUrlRejectedException;
use Modules\Plugins\Http\Requests\PluginUploadRequest;
...
class PluginController extends Controller
{
    ...
    /**
     * Deactivate an installed plugin and request an edge-cache purge.
     *
     * A dependency-blocked deactivation (an active plugin still depends on
     * this one) throws from the framework as of cms-framework 2.9.0; the
     * generic {@see Throwable} fallback keeps that from surfacing as a 500.
     */
    public function deactivate(string $slug): RedirectResponse
    {
        try {
            ...
            // A dependency-blocked deactivation (an active plugin still
            // depends on this one) now throws from the framework; keep the
            // operator-facing message generic for the same reason activate()
            // does — the report() call preserves the full detail.
            return back()->withErrors(['slug' => __('Failed to deactivate plugin.')]);
        }
