...
namespace Modules\Updater\Http\Controllers\Settings;
...
use Modules\Updater\Services\UpdateNotifier;
use Throwable;
 ...
 *   - POST /admin/settings/updates/check → drop that cached answer and
 *     redirect back into the GET, so an admin can see a just-published
 *     release without waiting out the TTL.
...
class UpdatesController extends Controller
{
    ...
    /**
     * Force a fresh release check, bypassing the cached result.
     *
     * `UpdateChecker` memoises its answer for `cms.updates.cache_ttl`
     * (12h by default), so a release published since the last check stays
     * invisible until the TTL lapses. That is the right default for the
     * dashboard banner and wrong for an admin who just published a release
     * and wants to see it — whose only recourse was
     * `php artisan update:check --clear-cache` over SSH.
     *
     * Deliberately does not render a result itself: it clears the cache and
     * redirects back into `show()`, so the freshly-fetched release travels
     * the same classification path as every other visit and there is only
     * one place where "what does the feed say" is turned into page state.
     *
     * Rate-limited at the route (`throttle:6,1`) for the same reason as
     * `plugins/check-updates`: every hit drops the cached answer and forces
     * a synchronous GitLab round-trip, so an admin leaning on the button
     * shouldn't be able to pin PHP workers.
     */
    public function check(ApplicationUpdateManager $manager): RedirectResponse
    {
        try {
            $manager->clearCache();

            // The banner's flag is derived from the same feed, so a manual
            // re-check has to invalidate it too — otherwise the page and the
            // dashboard disagree until the next scheduled tick. Inside the
            // same guard because it is the same cache store: whatever broke
            // `clearCache()` will usually break this too, and a 500 here
            // would strand the admin on an error page instead of the
            // Updates screen.
            Cache::forget('cms.update_available');
        } catch (Throwable $e) {
            Log::warning('Keystone update cache clear failed', [
                'exception' => $e->getMessage(),
            ]);

            // The driver's own message can name hosts, sockets, or
            // credentials. It is already in the log, where the operator who
            // needs it can reach it.
            return redirect()
                ->route('admin.settings.updates')
                ->with('error', 'Could not clear the cached update check. See the application log for details.');
        }

        Hooks::safeDoAction('keystone.cache.forgotten', 'cms.update_check');

        // Names only what this action actually did. The re-check itself
        // happens in `show()` after the redirect and can still fail — a
        // "Checked for updates." banner sitting above a check_error panel
        // would contradict itself.
        return redirect()
            ->route('admin.settings.updates')
            ->with('success', 'Cleared the cached update check.');
    }
