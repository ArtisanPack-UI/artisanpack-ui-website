...
import { rebaseEditBaseline, rebaseEditForm } from '@/lib/admin/rebaseEditForm';
...
import DistractionFreeBar from '@/components/admin/editor/DistractionFreeBar';
...
export default function Edit() {
    ...
    // Gate the primary Save until the visual editor has finished mounting
    // (issue #237): a click that lands mid-mount was silently dropped. When
    // this content type has no editor there is nothing to wait for, so Save
    // is live from first paint.
    const editorEnabled = has('editor');
    // Track WHICH record's editor has reported ready rather than a bare
    // boolean (issue #237). A same-component navigation to another record
    // re-boots VisualEditor (its effect keys on `id`), so deriving readiness
    // per-record re-gates Save until that new boot settles — no reset effect
    // needed, and no post-paint flicker. A save round-trip keeps the same id,
    // so the gate never drops mid-edit. Non-editor screens are always ready.
    const [readyRecordId, setReadyRecordId] = useState<number | null>(null);
    const editorReady = !editorEnabled || readyRecordId === post.id;
    ...
    // Editor chrome view mode (issue #239). Only applies when the visual
    // editor is actually present; posts have a right settings sidebar, so
    // distraction-free gets a slide-over.
    const viewModeControl = useEditorViewMode({
        postType: 'posts',
        initialMode: editorPreferences.view_mode,
        enabled: has('editor'),
        hasSettingsPanel: true,
    });
    const distractionFree = viewModeControl.mode === 'distraction-free';
    ...
    function submit(e: FormEvent | null, statusOverride?: StatusOption['value']) {
        ...
        router.put(update(post.id).url, scoped, {
            onSuccess: (page) => {
                // Rebase every persisted field from the fresh page props
                // rather than from the submitted snapshot. Some fields are
                // server-owned — publishing with "Immediately" sends
                // `published_at: null` and gets a real timestamp back, a
                // future date on Published comes back as Scheduled, a
                // colliding slug comes back adjusted — and a
                // `beforeSubmit` filter may have rewritten any of the
                // others. Either way the response, not the snapshot, is
                // what the database now holds; see lib/admin/rebaseEditForm
                // for the in-flight-edit guard that rides along with it.
                //
                // Read off `page`, not the component-scope `post`: this
                // closure captured the props from before the visit.
                const saved = formFromPost((page.props as unknown as PageProps).post);

                setForm((current) => rebaseEditForm(current, submittedForm, saved));
                setBaseline(rebaseEditBaseline(submittedForm, saved));
    ...
    return (
        <>
            ...
            <form onSubmit={submit} className="flex flex-col gap-4">
                {/*
                 * The page's sole <h1> normally lives (sr-only) inside
                 * PageHeader; distraction-free hides the header, so re-supply
                 * the heading here or the document would have none.
                 */}
                {distractionFree && (
                    <h1 className="sr-only">{post.title || 'Untitled'}</h1>
                )}
                {/*
                 * Distraction-free hides the whole editor header — the
                 * floating control below is the mode switch / exit while it
                 * is on. Every other mode keeps the header, switcher included.
                 */}
                {!distractionFree && (
                    <PageHeader
                ...
                <EditorPanelLayout
                    layout={layout}
                    panels={panelNodes}
                    viewMode={viewModeControl.mode}
                    settingsOpen={viewModeControl.settingsOpen}
                    onCloseSettings={viewModeControl.closeSettings}
                    sidebarTop={
                        <AdminEditSlot
                            slot="sidebar-top"
                            contentType="posts"
                        ...
                        <>
                            ...
                            {has('editor') && (
                                <VisualEditor
                                    ...
                                    onReady={() => setReadyRecordId(post.id)}
                                />
                            )}
            ...
            <DistractionFreeBar
                ...
                onChange={viewModeControl.setMode}
                settingsOpen={viewModeControl.settingsOpen}
                onOpenSettings={viewModeControl.openSettings}
                floatingButtonRef={viewModeControl.floatingButtonRef}
            />
            <AdminEditSlot
                slot="after-form"
                contentType="posts"
...
/**
 * The form shape a post record hydrates into, minus the `custom_fields`
 * overlay (the server ships those values on the `customFields` prop).
 * Shared by the initial mount and the post-save rebase so both read the
 * record the same way — including the null-to-empty-string normalization,
 * without which a nulled `excerpt` would come back as a spurious dirty
 * field.
 */
function formFromPost(post: AdminPost) {
    return {
        title: post.title,
        slug: post.slug,
        status: post.status,
