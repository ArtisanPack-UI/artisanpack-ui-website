 ...
 * (or homepage via the `site.homepageId` setting for `/`), picks a view
 * and a site-editor block tree through {@see ThemeTemplateLocator}, and
 * hands `$page` to the result.
 *
 * A Blade theme gets its own template; a block theme — which ships
 * `templates/*.html` and no Blade — gets Keystone's `block-theme`
 * layout. Either way the tree reaches `<x-ve-blocks>`, so the
 * visual-editor's Blade renderer + the template-part inliner handle the
 * actual block-tree → HTML pass.
 */
class PublicPageController extends Controller
{
    public function __construct(
        private ThemeManager $themeManager,
        private ThemeTemplateLocator $templateLocator,
    ...
    public function home(): View
    {
        ...
        if (null === $page) {
            // A fresh install has no published homepage, so `/` would
            // otherwise 404 — a Laravel-default error screen for a site
            // owner's very first click (see #116). Render a branded
            // placeholder pointing at the admin dashboard instead, so the
            // first impression is a "build your homepage" CTA rather than a
            // dead end. `show()` still 404s for genuinely unknown slugs.
            return view('no-homepage', [
    ...
    public function renderPage(Page $page, bool $isPreview = false): View
    {
        ...
        $template = $this->templateLocator->viewFor('page', $page->slug);
        ...
        $data = [
            'page' => $page,
            // Block themes render the page through their own
            // `templates/*.html` tree; jmwd-default and other Blade
            // themes ignore this and render `$page->getBlockContent()`
            // directly, so passing it unconditionally is safe.
            'templateBlocks' => $this->templateLocator->blocksFor('page', $page->slug),
