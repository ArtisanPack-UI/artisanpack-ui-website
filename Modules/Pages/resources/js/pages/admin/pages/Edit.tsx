...
export default function Edit() {
    ...
    // boolean — see posts/Edit.tsx for the rationale (issue #237).
    const [readyRecordId, setReadyRecordId] = useState<number | null>(null);
    const editorReady = !editorEnabled || readyRecordId === page.id;
    ...
    // Editor chrome view mode (issue #239) — see posts/Edit.tsx.
    ...
    function submit(e: FormEvent | null, statusOverride?: StatusOption['value']) {
        ...
        router.put(update(page.id).url, scoped, {
            onSuccess: (result) => {
                // See posts/Edit.tsx and lib/admin/rebaseEditForm: every
                // persisted field is rebased from the fresh page props,
                // because the server owns some of them outright and a
                // `beforeSubmit` filter may have rewritten the rest.
                //
                // Read off `result`, not the component-scope `page`: this
                // closure captured the props from before the visit.
                const saved = formFromPage((result.props as unknown as PageProps).page);
    ...
    return (
        <>
            ...
            <form onSubmit={submit} className="flex flex-col gap-4">
                {/* See posts/Edit.tsx — the sole <h1> lives in the header. */}
                {distractionFree && (
                    <h1 className="sr-only">{page.title || 'Untitled'}</h1>
                )}
                {/* See posts/Edit.tsx — distraction-free hides the header. */}
                {!distractionFree && (
                    <PageHeader
                        ...
                        <>
                            ...
                            {has('editor') && (
                                <VisualEditor
                                    ...
                                    onReady={() => setReadyRecordId(page.id)}
                                />
                            )}
...
/**
 * The form shape a page record hydrates into, minus the `custom_fields`
 ...
 * without which a nulled `excerpt` or `template` would come back as a
 * spurious dirty field.
 */
function formFromPage(page: AdminPage) {
    return {
        title: page.title,
        slug: page.slug,
        status: page.status,
