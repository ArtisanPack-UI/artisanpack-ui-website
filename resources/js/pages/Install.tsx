...
export default function Install({ siteTypes, defaults, result }: InstallProps) {
    ...
    // URL; the controller stamps a single-use session grant that the
    // middleware accepts for this POST, so we don't carry the token
    // back into the URL where it could leak through history, logs,
...
function ReviewStep({
    ...
    const siteTypeLabel =
        siteTypes.find((o) => o.value === form.data.site_type)?.label ??
        form.data.site_type;
