...
| `keystone.admin.settings.panels` | PHP | filter | Collects the module-owned panels of `/admin/settings` — emitted by `KeystoneShellController::settings()` through `App\Support\SettingsPanels::merge()`. Args: `(array<string, array<string, mixed>> $panels, array{surface: 'site'} $context)`; return the map with your panel added, keyed by the tab key the React page reads (`settings.privacy`). Claim with `??=`: panels are appended after core's in subscriber order, a key core already owns is never overwritten, and among modules the first claim wins. A non-array payload (or a non-string key) is dropped. Registered by `Modules\Privacy` and `Modules\Performance` from their service providers; a module whose provider never runs omits its key and the page drops that tab. |
...
| `keystone.admin.edit.form.beforeSubmit` | JS | filter | Fires from post / page Edit screens before `router.put(update…)`. Args: `(payload, { resource, id })`; return the (possibly rewritten) payload, or `false` to veto the submit. **Vetoing is silent by design** — the shell doesn't emit a toast or error, so a subscriber that returns `false` is responsible for its own user feedback (via `useToast()`, a modal, or by rewriting `payload` to a form the server will reject with a visible error). This lets a plugin veto for reasons the shell can't articulate on its behalf (unsaved changes elsewhere, custom validation, feature-flag gating). A rewritten field does not have to be re-applied on the way back: the screen rebases every persisted field from the save response, so what the server stored is what the form shows and what the next save re-sends. The one exception is a field the user edited while the save was in flight — that edit wins over the response and the form stays dirty, so a subscriber can't count on unconditional adoption. |
...
| `keystone.seo.aiScrapers.userAgents` | PHP | filter | Wraps the `Modules\Seo\Providers\SeoServiceProvider::AI_SCRAPER_USER_AGENTS` list before `config('seo.robots.rules')` is populated. Only applied when the `hide-ai-scrapers` visibility mode is on. Args: `(list<string> $userAgents)`; non-string / empty entries are dropped at the sink. |
